...
namespace wick
{
	...
	Pair::Pair(const Pairf& pairf)
	{
        x_ = floor((double) pairf.x_ + 0.5);
        y_ = floor((double) pairf.y_ + 0.5);
	}
	Pair::Pair()
	{
		x_ = 0;
	...
	Pairf Pair::operator+(const Pairf& other) const
	{
		return(Pairf(x_ + other.x_, y_ + other.y_));
	}
	Pairf Pair::operator-(const Pairf& other) const
	{
		return(Pairf(x_ - other.x_, y_ - other.y_));
