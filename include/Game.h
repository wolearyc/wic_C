...
namespace wick
{
    ...
    /** \brief holds and orchestrates the entire game and interfaces with the
      *        system
      *
      * Game creates the OS window, manages States, and handles keyboard and
      * mouse input. A Game should be instantiated (using 'new') in the
      * program's main method and a State should be added and initialized
      * to the Game. Then the Game should be executed. There should only be one 
      * Game object instantiated at any time. Game should be deallocated using 
      * the destructor before the program terminates.
      */
	class Game
	{
	public:
        /** \brief constructs Game with advanced settings
          * \param title the OS window title; must be a non-empty string
          * \param dimensions the frame dimensions in pixels; both components
          *        must be greater than 0
          * \param fps the target frames per second; must be greater than 0
          * \param resizeable whether or not the user can resize the OS window
          *		 by dragging or maximizing (ignored if the game is 
          *         fullscreen)
          * \param fullscreen whether or not the game is run fullscreen
          ...
          *            empty string;  title is corrected to "Wick Game"
          * \exception ParameterException non-fatal exception when either of the
          *            components in dimensions is less than or equal to 0; 
          *            invalid components are corrected to 500
          * \exception ParameterException non-fatal exception when fps equals 0;
          *            fps is corrected to 60
          */
        Game(string title, Pair dimensions, unsigned short fps,
             bool resizeable, bool fullscreen, unsigned short samples);
        /** \brief constructs Game at 60 fps with a non-resizeable
          *        non-fullscreen window using 3 sample antialiasing
          * \param title the OS window title; must be a non-empty string
          * \param dimensions the frame dimensions in pixels; both components
          *        must be greater than 0
          * \exception ParameterException non-fatal exception when title is an
        ...
        /** \brief deallocates Game
          *
          * The destructor deallocates all of the added States as well.
          */
        ~Game();
        /** \brief starts the game
          * 
          * This function exits once the user closes the OS window or stop() is
          * called.
          */
        void execute();
        ...
        /** \brief initializes a state that has already been added
          * \param id the ID assigned to the State
        ...
        /** \brief deallocates a State
          * \param id the ID assigned to the State
          * \exception WickException non-fatal exception when the ID is not
          *            assigned to any State; no state is deallocated
        ...
        /** \brief removes a State
          ...
          * leak. Deallocate the State first.
          * \param id the ID assigned to the State
        ...
        /** \brief Retrieves the keyboard input entered since the last frame
          * \return the input
          */
        string getInput();
        /** \brief retrieves the current location of the cursor
          * \return the coordinates of the cursor relative to the window
          */
